...
1. concave.points: radius_worst because concave points measure sharp inward curves on the tumor oundary whereas radius measures size. large lesions with many concvave points are strong malignancy signals. Note: use tensor products since the concave.point should not be included due to high VIF. 
2. concavity_worst x radius_worst: concavity measures depth/strent of indentations and large tumors with strong concavity tend to be malignant. This is one of the strongest known nonlinear effects in the WBCD dataset.
...
4. texture x concavity: Texture often reflects heteroeneity inside the lesion while concavity reflects edge irregularity. Malignant tumors with high concavity freqently have high texture variance due to necrosis and ductal extension. This interaction has bene noted in prvious ML analyses as important. 
```{r}

...
```{r}
# Just getting the data for the features
train2 <- as.data.frame(X_train) %>%
  mutate(
    diagnosis = diagnosis,   # add outcome
    prob = prob              # add predicted probability
  ) |>
  select(all_of(feature_list), diagnosis, prob)

train2$outcome <- ifelse(train2$diagnosis == "M", 1, 0)

test2 <- test |>
  select(all_of(feature_list), diagnosis)

test2$outcome <- ifelse(test2$diagnosis == "M", 1, 0)
...
```{r}
m_tp  <- gam(outcome ~ s(smoothness_se, bs = "tp", 
                           k = 10), 
            data = train2,
...
```{r}
m_cr  <- gam(outcome ~ s(smoothness_se, bs = "cr", 
                           k = 10), 
            data = train2,
            family = binomial(link = "logit"), 
            method = "REML")
...
```{r}
m_tp_sel  <- gam(outcome ~ s(smoothness_se, bs = "tp", 
                           k = 10), 
            data = train2,
            family = binomial(link = "logit"), 
            method = "REML",

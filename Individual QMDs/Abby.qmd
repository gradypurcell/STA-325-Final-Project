...
simulate_tumor <- function(radius_mean, concavity_worst,
                           n_points = 5000, x_offset = 0, seed = 1) {
  set.seed(seed)
  phi   <- runif(n_points, 0, pi)
  theta <- runif(n_points, 0, 2*pi)

  base_r  <- radius_mean
  bump_sd <- concavity_worst * 3   # tune this for more/less roughness
  ...
  data.frame(x = x, y = y, z = z, r = r)
}

# --- generate the two blobs (separated so they don't overlap) ---
rm_mal <- malignant_tumor$radius_mean
cw_mal <- malignant_tumor$concavity_worst
rm_ben <- benign_tumor$radius_mean
cw_ben <- benign_tumor$concavity_worst

offset <- max(rm_mal, rm_ben) * 2

tumor_mal <- simulate_tumor(rm_mal, cw_mal, x_offset = -offset, seed = 10)
tumor_ben <- simulate_tumor(rm_ben, cw_ben, x_offset =  offset, seed = 20)

# tumor_mal$type <- "Malignant (1)"
# tumor_ben$type <- "Benign (0)"
...
plot_ly() %>%
  add_trace(
    ...
    name = "Malignant (1)",
    marker = list(size = 2, opacity = 0.7, color = "red")
  ) %>%
  add_trace(
    data = tumor_ben,
    x = ~x, y = ~y, z = ~z,
    type = "scatter3d", mode = "markers",
    name = "Benign (0)",
    marker = list(size = 2, opacity = 0.7, color = "blue")
  ) %>%
  layout(
    title = "Simulated 3D Tumors: Malignant vs Benign",
    scene = list(
      xaxis = list(title = "X (mm)"),
      yaxis = list(title = "Y (mm)"),
      zaxis = list(title = "Z (mm)")
    ),
    legend = list(orientation = "h", x = 0.2, y = 1.05)
  )
...
ggplot(tumors, aes(x = x, y = y)) +
  ...
  labs(
    title = "Simulated Tumor Cross-Section: Benign vs Malignant",
